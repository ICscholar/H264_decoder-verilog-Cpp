...
// 1.根据当前4x4块的坐标,计算与其相邻的4x4块的坐标,包括上、左、上左等块。
// 2.判断这些相邻块是否存在(在图像范围内),把结果记录下来。
// 3.根据当前块和邻块的参考帧索引,从输入参数中提取相应的运动矢量。
// 4.如果当前块的参考帧索引与左、上、上右等块的参考帧索引不同,选择这些块中参考帧索引相同的块的运动矢量。
// 5.如果左上都不存在,选择左块的运动矢量。
// 6.对特殊情况16x8、8x16块进行处理。
// 7.如果上、上右都不存在,选择左块。其他情况下,采用中值滤波选择左、上、上右3块的中值作为输出。
