...
// 解析RBSP层传入的slice头数据,如slice类型、首个macroblock索引等。

// 处理macroblock级别的数据,如mb类型、qp值、运动矢量等。

// 驱动残差块数据的读取。

// 提供接口收集motion vector数据和nC值,供预测模块使用。

// 存储当前slice解析得到的数据到FPGA内存。

// 提供控制信号启动自适应掩模滤波等后续模块
`include "defines.v"
