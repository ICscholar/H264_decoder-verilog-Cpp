// 构建有限状态机，用于解析视频序列参数集(Sequence Parameter Set, SPS)中的视频可用性信息(VUI)部分。VUI提供了关于视频流的额外信息，如纵横比、视频格式、色彩描述等
// 在视频解码器的初始化阶段解析VUI参数，确保解码器能够根据视频流的特定要求（如时序信息、色彩空间等）正确配置。通过硬件实现这一过程，可以加速视频解码过程
