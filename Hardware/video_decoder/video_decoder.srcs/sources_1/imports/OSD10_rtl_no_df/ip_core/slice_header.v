// ' slice_header '接受与正在处理的当前切片相关的大量输入，包括序列参数集(SPS)和图像参数集(PPS)的标志和参数，以及解码器其他部分(如Exp-Golomb解码器)的输出。
// 它输出当前切片的几个关键参数，例如第一个宏块地址(' first_mb_in_slice ')、切片类型(' slice_type_mod5 ')、帧号(' frame_num ')，以及指示如何处理切片的各种标志

// Since H.264 defines slice types with values from 0 to 9, where some values (0-4) are for P and B slices and their equivalents (5-9) for their respective IDR (Instantaneous Decoder Refresh) versions, 
// this code maps these to a simplified `slice_type_mod5` to differentiate between the essential slice types without considering the IDR distinction at this level.
`include "defines.v"
